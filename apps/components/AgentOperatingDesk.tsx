...
export default function AgentOperatingDesk({ agentId, onBack }: { agentId: string, onBack: () => void }) {
  ...
  if (!agent || !agent.state) return (
    <div className="flex flex-col items-center justify-center h-full p-8 font-mono text-gray-500">
      ...
      <button onClick={onBack} className="mt-4 text-[#06F7C9] hover:text-white underline">Return to CrewAI</button>
    </div>
  );

  const { state } = agent;

  const handleModelAssign = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    const rawVal = e.target.value;
    const isDefault = rawVal === 'default-local';
    const finalVal = isDefault ? 'default' : rawVal;
    assignModelToAgent(agent.id, finalVal);

    try {
      await fetch('/api/agents/assign_model', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ agentId: agent.id, modelId: finalVal })
      });
    } catch (err) {
      console.error("Failed to sync assigned model to backend", err);
    }
  };
      ...
      <div className="p-4 md:p-6 border-b border-white/5 shrink-0 bg-[#050505]/40 backdrop-blur-md flex items-center justify-between">
        <div className="flex items-center gap-4">
           ...
           <div>
             <h2 className="text-lg font-bold text-white flex items-center gap-2 font-['Syne'] uppercase">
               <BrainCircuit size={18} className="text-[#06F7C9]" />
               {agent.name} 
               <span className="text-[10px] text-[red] text-gray-500 font-mono tracking-widest">{agent.id}</span>
               <span className="text-xs bg-[#06F7C9]/20 text-[#06F7C9] px-2 py-0.5 rounded border border-[#06F7C9]/30 ml-2 shadow-[0_0_10px_rgba(52,211,153,0.2)]">
                 Health: {state.health}%
               </span>
             </h2>
        ...
        <div className="flex gap-4">
           <div className="bg-white/5 border border-white/10 rounded-lg px-4 py-2 flex flex-col items-end">
              ...
              <span className="text-sm font-bold text-[#C026D3]">{state.performance.winRate.toFixed(1)}%</span>
           </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 pb-20 md:pb-0">
          ...
          <div className="lg:col-span-2 flex flex-col gap-4 md:gap-6">
            <Card className="flex flex-col min-h-[400px]">
              <CardHeader>
                <CardTitle className="flex items-center space-x-2"><Crosshair size={14} className="text-[#06F7C9]"/> <span>Decision Ledger & Diagnostics</span></CardTitle>
              </CardHeader>
                 ...
                 {state.decisionLog.length === 0 ? (
                   ...
                   <div className="flex flex-col gap-4">
                     {state.decisionLog.map(log => (
                       <div key={log.id} className="border border-white/10 rounded-lg bg-[#050505]/40 p-4">
                         <div className="flex justify-between items-center mb-3">
                           <div className="flex items-center gap-3">
                             <span className={clsx("px-2 py-0.5 rounded text-[10px] font-bold", log.type === 'LONG' ? 'bg-[#06F7C9]/20 text-[#06F7C9]' : log.type === 'SHORT' ? 'bg-red-500/20 text-red-400' : 'bg-gray-500/20 text-gray-400')}>
                               {log.type}
                             </span>
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                           ...
                           <div>
                             ...
                             <p className="text-xs text-gray-300"><span className="text-[#C026D3]">Lev:</span> {log.whyLeverage}</p>
                             <p className="text-xs text-gray-300 mt-1"><span className="text-[#C026D3]">Size:</span> {log.whySize}</p>
                           </div>
                           <div>
                             ...
                             <p className="text-xs text-gray-300 mt-1"><span className="text-[#06F7C9]">Target:</span> {log.whyTarget}</p>
                           </div>
                           <div className="flex flex-col justify-end">
                             <div className="bg-[#050505] border border-white/5 p-2 rounded flex justify-between items-center">
                                <span className="text-[10px] text-gray-500 uppercase">Swarm Consensus</span>
                                <div className="flex items-center gap-1">
                                  <BadgeCheck size={12} className="text-[#06F7C9]"/>
                                  <span className="text-xs font-bold text-[#06F7C9]">{log.confidence}%</span>
                                </div>
                             </div>
                           </div>
                         </div>
            ...
            <Card className="flex flex-col">
              <CardHeader>
                <CardTitle className="flex items-center space-x-2"><MemoryStick size={14} className="text-[#8B5CF6]"/> <span>Context & Memory State</span></CardTitle>
              </CardHeader>
          ...
          <div className="flex flex-col gap-4 md:gap-6">
            <Card className="flex flex-col">
              <CardHeader>
                <CardTitle className="flex items-center space-x-2"><Activity size={14} className="text-[#C026D3]"/> <span>Performance Analytics</span></CardTitle>
              </CardHeader>
              <CardContent className="p-4 flex flex-col gap-4">
                 <div className="flex justify-between items-center border-b border-white/5 pb-2">
                    <span className="text-xs text-gray-400">Total PnL</span>
                    <span className={clsx("text-sm font-bold", state.performance.pnl >= 0 ? "text-[#06F7C9]" : "text-red-400")}>
                 ...
                 <div className="text-xs space-y-4">
                    ...
                    <div className="flex justify-between items-center border-t border-white/5 pt-3">
                       ...
                       <select 
                         ...
                         onChange={handleModelAssign}
                         className="bg-[#050505]/40 border border-white/10 rounded px-2 py-1 text-xs text-white max-w-[150px]"
                       >
                         {models.map(m => (
                    ...
                    <div className="flex justify-between items-center border-t border-white/5 pt-3">
                       ...
                       <span className={clsx("px-2 py-0.5 rounded text-[10px]", agent.enabled ? 'bg-[#06F7C9]/20 text-[#06F7C9]' : 'bg-red-500/20 text-red-400')}>
